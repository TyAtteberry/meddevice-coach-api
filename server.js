...
async function getWorkspace(id){
  try{
    ...
    if(rows.length){const ws=rows[0];if(!ws.knowledge)ws.knowledge={};['product','clinicalStudies','competitive','objections','reimbursement','insights'].forEach(k=>{if(!ws.knowledge[k])ws.knowledge[k]=[];});return ws;}
  }catch(e){}
  // File fallback
