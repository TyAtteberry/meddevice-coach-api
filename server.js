...
// ══════════════════════════════════════════════════════════════
// WORKSPACES (unchanged — file-based)
// ══════════════════════════════════════════════════════════════
app.post('/workspaces',authMiddleware,(req,res)=>{
  const{name,company,repName,repEmail}=req.body;if(!name)return res.status(400).json({error:'Workspace name required'});
  const d=loadData();const id=generateCode(12);const inviteCode=generateCode(6);
  d.workspaces[id]={id,name,company:company||'',createdBy:repEmail||'',createdAt:Date.now(),inviteCode,members:[{name:repName||'',email:repEmail||'',role:'admin',joinedAt:Date.now()}],knowledge:{product:[],clinicalStudies:[],competitive:[],objections:[],reimbursement:[],insights:[]}};
  saveData(d);res.json({workspaceId:id,inviteCode});
});
app.post('/workspaces/join',authMiddleware,(req,res)=>{
  const{inviteCode,repName,repEmail}=req.body;const d=loadData();
  const ws=Object.values(d.workspaces).find(w=>w.inviteCode===inviteCode?.toUpperCase());
  ...
  saveData(d);res.json({workspaceId:ws.id,workspaceName:ws.name});
});
app.get('/workspaces/:id',authMiddleware,(req,res)=>{
  const d=loadData();const ws=d.workspaces[req.params.id];if(!ws)return res.status(404).json({error:'Not found'});
  if(!ws.knowledge.insights)ws.knowledge.insights=[];res.json(ws);
});
...
app.delete('/workspaces/:id/members/:email',authMiddleware,(req,res)=>{
  const{id}=req.params;const emailToRemove=decodeURIComponent(req.params.email);const d=loadData();const ws=d.workspaces[id];if(!ws)return res.status(404).json({error:'Not found'});
  const req_email=req.headers['x-requester-email'];const reqr=ws.members.find(m=>m.email===req_email);if(!reqr||reqr.role!=='admin')return res.status(403).json({error:'Only admins can remove members'});
  const target=ws.members.find(m=>m.email===emailToRemove);if(!target)return res.status(404).json({error:'Member not found'});if(target.role==='admin')return res.status(400).json({error:'Cannot remove admin'});
  ws.members=ws.members.filter(m=>m.email!==emailToRemove);saveData(d);res.json({removed:true,name:target.name});
});
app.post('/workspaces/:id/transfer',authMiddleware,(req,res)=>{
  const{id}=req.params;const{newAdminEmail,requesterEmail}=req.body;const d=loadData();const ws=d.workspaces[id];if(!ws)return res.status(404).json({error:'Not found'});
  ...
  saveData(d);res.json({transferred:true,newAdmin:na.name});
});

// ── KNOWLEDGE ─────────────────────────────────────────────────
app.post('/workspaces/:id/knowledge/:category',authMiddleware,(req,res)=>{
  const{id,category}=req.params;const d=loadData();const ws=d.workspaces[id];if(!ws)return res.status(404).json({error:'Not found'});
  if(!ws.knowledge[category])ws.knowledge[category]=[];
  const item={itemId:generateCode(8),...req.body,addedAt:Date.now()};ws.knowledge[category].push(item);saveData(d);res.json(item);
});
app.delete('/workspaces/:id/knowledge/:category/:itemId',authMiddleware,(req,res)=>{
  const{id,category,itemId}=req.params;const d=loadData();const ws=d.workspaces[id];if(!ws)return res.status(404).json({error:'Not found'});
  ws.knowledge[category]=(ws.knowledge[category]||[]).filter(i=>i.itemId!==itemId);saveData(d);res.json({deleted:true});
});

app.post('/workspaces/:id/knowledge/:category/upload',authMiddleware,upload.single('file'),async(req,res)=>{
  const{id,category}=req.params;const d=loadData();const ws=d.workspaces[id];
  if(!ws)return res.status(404).json({error:'Not found'});if(!req.file)return res.status(400).json({error:'No file'});
  try{
    const b64=req.file.buffer.toString('base64');
    const cr=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:2000,messages:[{role:'user',content:[{type:'document',source:{type:'base64',media_type:'application/pdf',data:b64}},{type:'text',text:getExtractionPrompt(category)}]}]})});
    ...
    if(!ws.knowledge[category])ws.knowledge[category]=[];ws.knowledge[category].push(item);saveData(d);res.json({item,needsInterview:extracted.gaps?.length>0});
  }catch(e){res.status(500).json({error:e.message});}
});
...
app.post('/workspaces/:id/knowledge/:category/url',authMiddleware,async(req,res)=>{
  ...
  const d=loadData();const ws=d.workspaces[id];if(!ws)return res.status(404).json({error:'Not found'});
  ...
  try{
    ...
    if(!ws.knowledge[category])ws.knowledge[category]=[];ws.knowledge[category].push(item);saveData(d);res.json({item});
  }catch(e){
...
app.post('/workspaces/:id/insights/generate',authMiddleware,async(req,res)=>{
  ...
  const d=loadData();const ws=d.workspaces[id];if(!ws)return res.status(404).json({error:'Not found'});
  const k=ws.knowledge;let kc='';
  if(k.product?.length)kc+=`PRODUCT:\n${k.product.map(p=>`- ${p.title||''}: ${p.summary||''}`).join('\n')}\n\n`;
  if(k.clinicalStudies?.length)kc+=`STUDIES:\n${k.clinicalStudies.map(s=>`- ${s.title||''}: ${s.keyFindings||''}`).join('\n')}\n\n`;
  if(k.competitive?.length)kc+=`COMPETITIVE:\n${k.competitive.map(c=>`- ${c.competitorName||''}: ${(c.keyWeaknesses||[]).join('; ')}`).join('\n')}\n\n`;
  try{
    const cr=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:2500,messages:[{role:'user',content:`Challenger sales insight for medical device rep.\nPRODUCT: ${product||''} SPECIALTY: ${specialty||''}\n${kc}\nReturn ONLY valid JSON:{"title":"...","targetSpecialty":"${specialty||''}","product":"${product||''}","problemStatement":"...","reframe":"...","evidence":"...","emotionalImpact":"...","newWayForward":"...","solution":"...","bestPersonas":["skeptic"],"talkingPoints":["..."],"proofPoints":["..."],"commonObjections":["..."],"scope":"${scope||'team'}"}` }]})});
    const cd=await cr.json();const raw=cd.content?.map(b=>b.text||'').join('\'')||''
    let insight={};try{const jm=raw.match(/\{[\s\S]*\}/);if(jm)insight=JSON.parse(jm[0]);}catch(e){return res.status(500).json({error:'Parse error'});}
    const item={itemId:generateCode(8),...insight,addedAt:Date.now(),source:'ai_generated'};
    if(!ws.knowledge.insights)ws.knowledge.insights=[];ws.knowledge.insights.push(item);saveData(d);res.json(item);
  }catch(e){res.status(500).json({error:e.message});}
});
...
app.post('/workspaces/:id/competitive-analysis',authMiddleware,async(req,res)=>{
  ...
  try{
    ...
    if(!ws.knowledge.competitive)ws.knowledge.competitive=[];ws.knowledge.competitive.push(item);saveData(d);res.json(item);
  }catch(e){res.status(500).json({error:e.message});}
});
...
function getExtractionPrompt(category){
  const p={
    clinicalStudies:`Extract clinical study info. Return ONLY valid JSON: {"title":"","authors":"","year":"","journal":"","studyType":"","sampleSize":"n=X","patientPopulation":"","primaryEndpoint":"","keyFindings":"2-3 sentences","outcomes":"data with numbers","clinicalImplication":"","limitations":"","salesInsight":"","gaps":[]}`,
