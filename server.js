// ══════════════════════════════════════════════════════════════
// Conceromed — Backend Server v4.24
...
// Claude model — single source of truth. Override via Railway env var CLAUDE_MODEL
// without a code deploy when Anthropic retires a model. Retired 2026-06-15:
// claude-sonnet-4-20250514 / claude-sonnet-4-0 (both return errors).
const CLAUDE_MODEL         = process.env.CLAUDE_MODEL         || 'claude-sonnet-4-6';
const RETIRED_MODELS = [/^claude-sonnet-4-(0|20250514)$/, /^claude-opus-4-(0|20250514)$/];
...
app.use(cors({ origin: '*', methods: ['GET','POST','PUT','PATCH','DELETE','OPTIONS'], allowedHeaders: ['Content-Type','x-session-token','x-admin-secret','x-rep-email','x-requester-email','x-user-email'] }));
...
// 20MB raw PDF. Base64 inflates ~33% (20MB -> ~27MB), staying under Anthropic's
// ~32MB request ceiling. Do not raise past ~23MB without chunking the document.
...
async function sbUpsert(table, body) {
  // Strip null values — Supabase schema cache rejects nulls for columns it hasn't seen yet
  // Only send fields with actual values (undefined and null excluded, except explicit nulls
  // needed for clearing fields — those use a separate patch approach)
...
// Patch a single row — safe for nullable column updates including explicit null clears
...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'4.24'}));
...
app.post('/auth/verify-otp', async (req, res) => {
  ...
  try {
    ...
    const sessionToken = makeEmailSessionToken(userEmail);
    SESSION_STORE.set(sessionToken, { createdAt: Date.now(), email: userEmail, supabaseUid: data.user?.id });
    res.json({
      token: sessionToken,
      email: userEmail,
...
app.post('/auth/refresh', async (req, res) => {
  ...
  try {
    ...
    const mlEmail = data.user?.email||'';
    const sessionToken = makeEmailSessionToken(mlEmail);
    SESSION_STORE.set(sessionToken, { createdAt: Date.now(), email: mlEmail });
...
app.post('/auth/login',(req,res)=>{
  ...
  const token=makeStatelessToken();res.json({token}); // stateless — survives redeploys
});
...
// Email-based stateless token — unique per user, survives restarts
function makeEmailToken(email){return crypto.createHmac('sha256',ADMIN_PASSWORD).update('mdsc-user-'+email).digest('hex').substring(0,32);}
function isEmailToken(t){
  // Check SESSION_STORE first (fast path)
  if(SESSION_STORE.has(t)) return SESSION_STORE.get(t)?.email||'unknown';
  // Brute-force check against known emails is impractical — instead store email in token itself
  // Token format: first 32 chars are HMAC, next part is base64 email
  try{
    const parts = t.split('.');
    if(parts.length===2){
      const email=Buffer.from(parts[1],'base64').toString('utf8');
      const expected=crypto.createHmac('sha256',ADMIN_PASSWORD).update('mdsc-user-'+email).digest('hex').substring(0,32);
      if(parts[0]===expected) return email;
    }
  }catch(e){}
  return null;
}
function makeEmailSessionToken(email){
  const hmac=crypto.createHmac('sha256',ADMIN_PASSWORD).update('mdsc-user-'+email).digest('hex').substring(0,32);
  return hmac+'.'+Buffer.from(email).toString('base64');
}
function authMiddleware(req,res,next){
  const t=req.headers['x-session-token'];
  if(!t) return res.status(401).json({error:'Not authenticated'});
  if(SESSION_STORE.has(t)||isStatelessToken(t)) return next();
  // Check email-based stateless token
  const emailFromToken=isEmailToken(t);
  if(emailFromToken) return next();
...
async function getWorkspace(id){
  ...
  // product + journeyTemplates live inside the knowledge JSONB blob so no
  // Supabase schema migration is needed. Surfaced at top level for the client.
  ws.product = ws.knowledge._product || r.product || '';
  ws.journeyTemplates = ws.knowledge._journeyTemplates || [];
...
async function saveWorkspace(ws){
  const knowledge={...(ws.knowledge||{})};
  // Fold the top-level convenience fields back into the JSONB blob before write.
  if(ws.product!==undefined)knowledge._product=ws.product||'';
  if(ws.journeyTemplates!==undefined)knowledge._journeyTemplates=ws.journeyTemplates||[];
  await sbUpsert('workspaces',{
    id:ws.id,name:ws.name,company:ws.company||'',created_by:ws.createdBy||'',
    invite_code:ws.inviteCode,members:ws.members||[],knowledge,
    updated_at:new Date().toISOString()
  });
}
...
// ── RENAME WORKSPACE ──────────────────────────────────────────
// Frontend (index.html ~4479) PUTs {name, product, company}. Was missing -> 404.
app.put('/workspaces/:id/rename',authMiddleware,async(req,res)=>{
  const{name,product,company}=req.body;
  const ws=await getWorkspace(req.params.id);
  if(!ws)return res.status(404).json({error:'Not found'});
  if(name!==undefined&&String(name).trim())ws.name=String(name).trim();
  if(product!==undefined)ws.product=String(product||'').trim();
  if(company!==undefined)ws.company=String(company||'').trim();
  await saveWorkspace(ws);
  res.json({renamed:true,id:ws.id,name:ws.name,product:ws.product,company:ws.company});
});

// ── JOURNEY TEMPLATES ─────────────────────────────────────────
// Frontend (index.html ~12405/12431/12451) expects:
//   GET    -> {templates:[...]}
//   POST   -> {template} in, {template} out (upsert by id)
//   DELETE -> {deleted:true}
app.get('/workspaces/:id/journey-templates',authMiddleware,async(req,res)=>{
  const ws=await getWorkspace(req.params.id);
  if(!ws)return res.status(404).json({error:'Not found'});
  res.json({templates:ws.journeyTemplates||[]});
});

app.post('/workspaces/:id/journey-templates',authMiddleware,async(req,res)=>{
  const{template}=req.body;
  if(!template||!template.productName)return res.status(400).json({error:'template.productName required'});
  const ws=await getWorkspace(req.params.id);
  if(!ws)return res.status(404).json({error:'Not found'});
  if(!ws.journeyTemplates)ws.journeyTemplates=[];
  const t={
    ...template,
    id: template.id || generateCode(8),
    acquisitionSteps: template.acquisitionSteps||[],
    adoptionBuckets: template.adoptionBuckets||[],
    updatedAt: Date.now(),
  };
  const idx=ws.journeyTemplates.findIndex(x=>x.id===t.id);
  if(idx>=0)ws.journeyTemplates[idx]={...ws.journeyTemplates[idx],...t};
  else{t.createdAt=Date.now();ws.journeyTemplates.push(t);}
  await saveWorkspace(ws);
  res.json({template:idx>=0?ws.journeyTemplates[idx]:t});
});

app.delete('/workspaces/:id/journey-templates/:templateId',authMiddleware,async(req,res)=>{
  const ws=await getWorkspace(req.params.id);
  if(!ws)return res.status(404).json({error:'Not found'});
  const before=(ws.journeyTemplates||[]).length;
  ws.journeyTemplates=(ws.journeyTemplates||[]).filter(t=>t.id!==req.params.templateId);
  await saveWorkspace(ws);
  res.json({deleted:before!==ws.journeyTemplates.length});
});
...
app.post('/workspaces/:id/knowledge/:category/upload',authMiddleware,upload.single('file'),async(req,res)=>{
  ...
  if(adminEmail){
    try{
      ...
      docLimit=isFounder?999:(200+bonus); // founding members get unlimited
    }catch(e){}
  }
...
app.post('/workspaces/:id/knowledge/:category/url',authMiddleware,async(req,res)=>{
  ...
  try{
    ...
    const cr=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:CLAUDE_MODEL,max_tokens:2000,messages:[{role:'user',content:`${getExtractionPrompt(category)}\n\nSOURCE: ${url}\n\nCONTENT:\n${text}`}]})});
    const cd=await cr.json();const raw=cd.content?.map(b=>b.text||'').join('')||'';
...
app.post('/accounts/:accountId/activity/draft-email',authMiddleware,async(req,res)=>{
  ...
  try{
    const cr=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:CLAUDE_MODEL,max_tokens:1200,messages:[{role:'user',content:`Write a medical device sales email.\nTYPE: ${typeGuide[emailType]||typeGuide.followup}\nACCOUNT: ${accountName||''} CLINICIAN: ${clinician||''} (${clinicianTitle||''})\nREP: ${repName||''} at ${repCompany||''} selling ${repProducts||''}\nCALL SUMMARY: ${callSummary||''}\nWHAT RESONATED: ${whatResonated||''}\nOBJECTIONS: ${objections||''}\nNEXT STEP: ${nextStep||''}\nADDITIONAL: ${additionalContext||''}\nReturn ONLY valid JSON:{"subject":"compelling subject","body":"full email body"}`}]})});
    const cd=await cr.json();const raw=cd.content?.map(b=>b.text||'').join('\'')||''
...
async function handleExtractActions(req,res){
  ...
  try{
    const cr=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:CLAUDE_MODEL,max_tokens:800,messages:[{role:'user',content:`Extract action items from this medical device email.\nACCOUNT:${accountName||''} CLINICIAN:${clinician||''}\nEMAIL:\n${content||''}\nReturn ONLY valid JSON:{"actionItems":[{"text":"specific action","owner":"rep|physician|clinic","priority":"high|medium|low","category":"follow_up|clinical|admin","suggestedDueDate":"YYYY-MM-DD or null"}],"summary":"what this email accomplished"}`}]})});
    const cd=await cr.json();const raw=cd.content?.map(b=>b.text||'').join('')||'';
...
app.post('/accounts/:accountId/activity/quick-log',authMiddleware,async(req,res)=>{
  ...
  if(enrich){
    try{
      const cr=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:CLAUDE_MODEL,max_tokens:600,messages:[{role:'user',content:`Clean up this voice-logged call note.\nRAW: ${rawNote}\nReturn ONLY valid JSON:{"title":"5-10 word title","cleanedNote":"polished version","nextStep":"single next action or empty"}`}]})});
      const cd=await cr.json();const raw=cd.content?.map(b=>b.text||'').join('\'')||''
...
app.post('/rep/daily-brief',authMiddleware,async(req,res)=>{
  ...
  let acctCtx='';
  const {staleAccounts,pipelineSummary}=req.body;
  if(accounts?.length){
    acctCtx=`ACCOUNTS:\n${accounts.slice(0,8).map(a=>{
      ...
      const pl=a.pipeline;
      const stage=a.stage;
      const plStr=pl?` | Pipeline: ${pl.probability}% probability · $${pl.weightedValue>=1000?Math.round(pl.weightedValue/1000)+'K':pl.weightedValue} weighted${pl.monthsToClose?' · ~'+pl.monthsToClose+'mo to close':''}`:''
      const stageStr=stage?` | ${stage.phase==='adoption'?'ADOPTION phase':'Acquisition step '+stage.stepNum+'/'+stage.totalSteps+(stage.stepLabel?' — next: '+stage.stepLabel:'')}`:''
      return '- '+a.name+(a.location?' ('+a.location+')':'')+(status?' ['+status+']':'')+stageStr+(nca?' | Next customer action: '+nca+(ncaDate?' by '+ncaDate:''):(!nca&&nextStep?' | Next step: '+nextStep:''))+plStr;
    }).join('\n')}`;
  }
  // Stale stage alerts
  let staleCtx='';
  if(staleAccounts?.length){
    staleCtx=`\nSTALE STAGE ALERTS (accounts stuck longer than expected):\n${staleAccounts.map(s=>`- ${s.accountName}: stuck at "${s.stepLabel}" for ${s.weeksAtStage} weeks (typical: ${s.expectedWeeks} weeks) — mention this in the brief and suggest the rep identify the blocker`).join('\n')}`;
  }
  // Pipeline summary
  let pipelineCtx='';
  if(pipelineSummary?.count){
    pipelineCtx=`\nTERRITORY PIPELINE: ${pipelineSummary.count} accounts tracked · Weighted value: $${pipelineSummary.totalWeighted>=1000?Math.round(pipelineSummary.totalWeighted/1000)+'K':pipelineSummary.totalWeighted} · High-probability accounts: ${pipelineSummary.highCount}`;
  }
...
app.post('/proxy/claude',authMiddleware,async(req,res)=>{
  // GitHub Pages caches aggressively, so an old index.html can keep sending a
  // retired model string long after deploy. Rewrite it here rather than 500.
  const body={...req.body};
  if(!body.model||RETIRED_MODELS.some(rx=>rx.test(body.model))){
    if(body.model)console.log(`[proxy/claude] rewriting retired model ${body.model} -> ${CLAUDE_MODEL}`);
    body.model=CLAUDE_MODEL;
  }
  try{const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify(body)});res.json(await r.json());}catch(e){res.status(500).json({error:e.message});}
});
...
app.post('/admin/comp', adminAuth, async (req, res) => {
  ...
  try {
    ...
    // Auto-send welcome email for all tiers except founding_10 (Ty handles those personally)
    if (tier !== 'founding_10' && RESEND_API_KEY) {
      const tierLabels = { founding_40:'founding member', comp_30:'30-day complimentary', comp_90:'90-day complimentary', comp_custom:'complimentary', paid:'Pro' };
      const tierLabel = tierLabels[tier] || 'Pro';
      await sendEmail({
        to: email,
        subject: `You\'ve been granted access to MedDeviceSalesPro`,
        html: buildAccessGrantedEmail(email, tierLabel, expiry),
        text: `You\'ve been granted ${tierLabel} access to MedDeviceSalesPro. Get started at meddevicesalespro.com — log in with this email address.`,
      }).catch(e => console.error('[ADMIN COMP] email error:', e.message));
    }
...
app.post('/admin/proscan-invite', adminAuth, async (req, res) => {
  ...
  if (!email) return res.status(400).json({ error: 'email required' });
  try {
    const existing = await sbGet('rep_profiles', `?email=eq.${encodeURIComponent(email)}&limit=1`);
    if (existing && existing.length > 0) {
      // Row exists — use PATCH so we can explicitly set null to clear the field
      await sbPatch('rep_profiles', `?email=eq.${encodeURIComponent(email)}`, {
        proscan_invited_at: sent ? new Date().toISOString() : null,
        updated_at: new Date().toISOString(),
      });
    } else {
      // No row yet — insert with only non-null fields
      const insertBody = { email, name: '', updated_at: new Date().toISOString(), created_at: new Date().toISOString() };
      if (sent) insertBody.proscan_invited_at = new Date().toISOString();
      await sbUpsert('rep_profiles', insertBody);
    }
    console.log(`[ADMIN PROSCAN INVITE] ${email} → ${sent ? 'sent' : 'cleared'}`);
    res.json({ success: true, email, sent });
  } catch(e) {
    console.error('[ADMIN PROSCAN INVITE] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
// ── WELCOME EMAIL ────────────────────────────────────────────
function buildAccessGrantedEmail(email, tierLabel, expiry) {
  const expiryLine = expiry
    ? `<p style="font-size:13px;color:#888;margin:8px 0 0;">Your access is active until ${new Date(expiry).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}.</p>`
    : `<p style="font-size:13px;color:#888;margin:8px 0 0;">Your access has no expiration date.</p>`;
  return `<!DOCTYPE html>
<html>
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  <tr><td style="background:#0a0c0f;border-radius:12px 12px 0 0;padding:32px 40px;text-align:center;">
    <p style="color:#00c9a7;font-size:11px;letter-spacing:3px;text-transform:uppercase;margin:0 0 8px;">You're in —</p>
  ...
  <tr><td style="background:#ffffff;padding:40px;">
    <p style="font-size:16px;color:#1a1a2e;margin:0 0 16px;">Hi,</p>
    <p style="font-size:15px;color:#333;line-height:1.7;margin:0 0 16px;">You've been granted <strong>${tierLabel} access</strong> to MedDeviceSalesPro — an AI coaching platform built specifically for medical device reps.</p>
    ${expiryLine}
    <p style="font-size:15px;color:#333;line-height:1.7;margin:16px 0 24px;">Here's how to get started:</p>
    ...
    <tr>
      ...
      <td style="padding-left:12px;">
        <p style="font-size:15px;font-weight:700;color:#1a1a2e;margin:0 0 4px;">Log in with this email address</p>
        <p style="font-size:14px;color:#555;line-height:1.6;margin:0;">Go to <a href="https://www.meddevicesalespro.com" style="color:#00c9a7;font-weight:600;">meddevicesalespro.com</a> and enter your email. You'll receive a magic link — click it and you're in. No password to remember.</p>
      </td>
    </tr>
    </table>
    ...
    <tr>
      ...
      <td style="padding-left:12px;">
        ...
        <p style="font-size:14px;color:#555;line-height:1.6;margin:0;">This is the most important first step. Your ProScan tells the AI how you're wired — your strengths, stress patterns, and blind spots. Every coaching session gets personalized to <em>you</em>. Check your email for your ProScan invite, or upload an existing report from your profile settings.</p>
      </td>
    </tr>
    </table>
    ...
    <tr>
      ...
      <td style="padding-left:12px;">
        <p style="font-size:15px;font-weight:700;color:#1a1a2e;margin:0 0 4px;">Set Your 90-Day Goals & Add Key Accounts</p>
        <p style="font-size:14px;color:#555;line-height:1.6;margin:0;">Block 20 minutes. Set what winning this quarter looks like, then add your 3–5 most important accounts. The AI references both in every pre-call brief, role play, and daily coaching session.</p>
      </td>
    </tr>
    </table>
    ...
    <tr>
      ...
      <td style="padding-left:12px;">
        ...
        <p style="font-size:14px;color:#555;line-height:1.6;margin:0;">Upload clinical studies, competitive intel, product materials, and objection responses in the Field Intelligence tab. This becomes the foundation for your role plays and commercial insights.</p>
      </td>
    </tr>
    </table>
    ...
    <p style="font-size:14px;color:#555;line-height:1.7;margin:0 0 16px;padding-top:20px;border-top:1px solid #eee;">Reply to this email anytime — you'll reach me directly. I want to know how it's working for you.</p>
    <p style="font-size:15px;color:#333;margin:0;">— Ty Atteberry<br><span style="color:#888;font-size:13px;">Founder, Conceromed</span><br><span style="color:#aaa;font-size:12px;font-style:italic;">Nearly 30 years in medical device sales · Medtronic, Abbott &amp; startups</span></p>
  </td></tr>
  <tr><td style="background:#f4f6f8;border-radius:0 0 12px 12px;padding:20px 40px;text-align:center;">
    ...
    <p style="color:#aaa;font-size:11px;margin:4px 0 0;">You received this because access was granted to this email address.</p>
  </td></tr>
</table>
</td></tr>
</table>
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <tr><td style="background:#ffffff;padding:40px;">
    ...
    <p style="font-size:15px;color:#333;margin:0;">&#8212; Ty Atteberry<br><span style="color:#888;font-size:13px;">Founder, Conceromed</span><br><span style="color:#aaa;font-size:12px;font-style:italic;">Nearly 30 years in medical device sales &middot; Medtronic, Abbott &amp; startups bringing new technologies to market</span></p>
  </td></tr>

  <!-- Footer -->
  <tr><td style="background:#f4f6f8;border-radius:0 0 12px 12px;padding:20px 40px;text-align:center;">
    <p style="color:#aaa;font-size:11px;margin:0;">MedDeviceSalesPro is a product of Conceromed LLC &middot; <a href="https://www.meddevicesalespro.com" style="color:#aaa;">meddevicesalespro.com</a></p>
...
// Without this, multer's LIMIT_FILE_SIZE surfaces as an opaque 500.
app.use((err,req,res,next)=>{
  if(err&&err.name==='MulterError'){
    const msg=err.code==='LIMIT_FILE_SIZE'
      ? `File too large — maximum is ${MAX_UPLOAD_MB}MB. Try splitting the PDF or removing embedded images.`
      : `Upload error: ${err.message}`;
    console.log(`[upload] ${err.code} on ${req.originalUrl}`);
...
const server=app.listen(PORT,()=>console.log(`Conceromed API v4.24 running on port ${PORT}`));
