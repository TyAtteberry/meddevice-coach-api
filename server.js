...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'4.1'}));
...
Return ONLY valid JSON with no markdown or backticks:
{
  ...
  "traitRangeAnalysis": {
    "overview": "Plain language explanation of what the deviation/range framework means for this specific person — written for the rep, not clinical. Cover: each trait has an authentic operating range of roughly ±0.5 from their score. They can dial up or down within that window but cannot sustain behavior outside it without energy cost. The farther from midline (midline = ~3.5 on the 1-7 scale), the more that trait dominates their natural presentation and the harder it is to mask.",
    "midlineNote": "If any traits are near midline (score 3-5), note this: they have natural flexibility on that trait but watch-out is that teams may not always know where they stand — perceived as unpredictable or inconsistent. Being near midline means you can appear dominant OR non-dominant depending on situation, which is a strength but requires intentional signaling.",
    "traits": {
      "dominance": {
        "score": 1-7,
        "authenticRange": "score minus 0.5 to score plus 0.5 e.g. 1.5-2.5",
        "distanceFromMidline": "far below|below|near|above|far above — midline is 3.5",
        "canCrossMidline": true,
        "flexibilityDesc": "What this range means — can they appear dominant sometimes? Always appear dominant? Never appear dominant? Be specific.",
        "salesImplication": "How this trait range plays out specifically in medical device sales calls — what they naturally do, what they cannot fake",
        "watchOut": "The specific behavioral watch-out for this score on this trait"
      },
      "extroversion": {
        "score": 1-7,
        "authenticRange": "e.g. 5.5-6.5",
        "distanceFromMidline": "far below|below|near|above|far above",
        "canCrossMidline": false,
        "flexibilityDesc": "What this range means for their social/communication behavior",
        "salesImplication": "How extroversion range plays out in sales — relationship building, cold calls, group presentations",
        "watchOut": "The specific watch-out for this score"
      },
      "pace": {
        "score": 1-7,
        "authenticRange": "e.g. 3.0-4.0",
        "distanceFromMidline": "far below|below|near|above|far above",
        "canCrossMidline": true,
        "flexibilityDesc": "What this pace range means — urgent vs steady, how much they can adjust",
        "salesImplication": "How pace plays out — follow-up cadence, handling delays, urgency creation",
        "watchOut": "The specific watch-out"
      },
      "conformity": {
        "score": 1-7,
        "authenticRange": "e.g. 5.5-6.5",
        "distanceFromMidline": "far below|below|near|above|far above",
        "canCrossMidline": false,
        "flexibilityDesc": "What this conformity range means — detail orientation, rule following, accuracy needs",
        "salesImplication": "How conformity plays out — prep depth, handling ambiguity, documentation habits",
        "watchOut": "The specific watch-out"
      }
    }
  },
  ...
  "traitPairAnalysis": [
    {
      "pairName": "pair name from report e.g. Persuasive/Seller",
      "traitsInvolved": "e.g. Extroversion over Dominance",
      "defaultBehavior": "What this person naturally defaults to doing because of this combination — be very specific and behavioral",
      "strengthInSales": "Where this pair is a genuine competitive advantage in medical device sales",
      "blindSpot": "The specific blind spot this pair creates — what they will consistently do wrong or struggle with",
      "challengerImplication": "How this trait pair specifically affects their ability to execute Challenger selling — which stage(s) will feel natural vs. which will feel uncomfortable",
      "watchOutExample": "A concrete real-world example of this blind spot playing out in a sales or leadership situation"
    }
  ],

  "challengerFrictionMap": {
    "overview": "How this rep's specific profile predicts their Challenger selling strengths and friction points. The Challenger methodology requires 6 stages: Credibility Builder, Reframe, Rational Drowning, Emotional Impact, New Way Forward, Solution. Each trait creates natural affinity with some stages and friction at others.",
    "naturalStages": ["stage names where their wiring gives them a natural advantage"],
    "frictionStages": [
      {
        "stage": "stage name e.g. The Reframe",
        "whyItsFriction": "Specifically why this rep's wiring makes this stage hard — connect directly to their trait scores",
        "whatTheyWillDo": "The specific behavioral mistake this rep will make at this stage — what 'pulling the punch' looks like for them",
        "coachingCue": "The one specific thing to do/remember at this exact moment — concrete and actionable",
        "whatItFeelsLike": "What the discomfort will feel like from the inside — so they can recognize it in the moment"
      }
    ],
    "dominanceImplication": "How their D score affects Challenger — High D tends to skip Emotional Impact and rush to solution. Low D may struggle with the assertiveness required in the Reframe.",
    "extroversionImplication": "How their E score affects Challenger — High E tends to soften the Reframe to avoid discomfort. Low E may deliver the Reframe too coldly without the relationship warmth needed.",
    "paceImplication": "How their Pace score affects Challenger — High Pace may rush through Rational Drowning. Steady Pace may linger too long and lose momentum.",
    "conformityImplication": "How their C score affects Challenger — High C may over-drown with data in Rational Drowning. Low C may skip the evidence entirely and lose credibility.",
    "repSpecificAdvice": "2-3 sentences of the most important Challenger coaching insight for THIS specific person given their complete profile — the thing their coach would tell them before every call"
  },

  "customerReadingGuide": {
    "overview": "Most people prefer to be communicated with the way they are wired. This rep's natural style will click instantly with some physician types and create friction with others. Knowing this before walking in changes everything.",
    "naturalFitPhysician": "Description of the physician type this rep will naturally connect with — what their wiring looks like, how the conversation will flow",
    "frictionPhysician": "Description of the physician type that will create the most friction — what their wiring looks like and why it clashes",
    "adaptationStrategy": "How to read a physician's wiring in the first 2 minutes of a call and what to adjust",
    "highDPhysicianTip": "How to communicate with a high-Dominance physician — they want direct, efficient, conclusory. Skip the why.",
    "highEPhysicianTip": "How to communicate with a high-Extroversion physician — they want connection, stories, impact on people.",
    "highCPhysicianTip": "How to communicate with a high-Conformity physician — they want evidence, process, accuracy. Earn trust with data.",
    "steadyPacePhysicianTip": "How to communicate with a steady-Pace physician — they need time to process, don't rush the close."
  },
...
app.post('/rep/daily-brief',authMiddleware,async(req,res)=>{
  ...
  if(proscan){
    ...
    // Trait range analysis
    const tra=proscan.traitRangeAnalysis||{};
    const traTraits=tra.traits||{};
    const traitRangeCtx=tra.overview?`
TRAIT OPERATING RANGES (each trait ±0.5 is authentic range — outside = energy cost):
${Object.entries(traTraits).map(([t,v])=>`${t.toUpperCase()}: score ${v.score||'?'} | authentic range ${v.authenticRange||'?'} | ${v.distanceFromMidline||''} from midline${v.canCrossMidline?' (can cross midline)':' (cannot cross midline)'}
  Sales: ${v.salesImplication||''} | Watch-out: ${v.watchOut||''}`).join('\n')}
${tra.midlineNote||''}`:''

    // Trait pair analysis
    const tpa=proscan.traitPairAnalysis||[];
    const traitPairCtx=tpa.length?`
TRAIT PAIRS (how trait combinations create behavioral defaults):
${tpa.map(p=>`${p.pairName}: ${p.traitsInvolved||''}
  Default behavior: ${p.defaultBehavior||''}
  Sales strength: ${p.strengthInSales||''}
  Blind spot: ${p.blindSpot||''}
  Challenger: ${p.challengerImplication||''}
  Watch-out example: ${p.watchOutExample||''}`).join('\n')}`:''

    // Challenger friction map
    const cfm=proscan.challengerFrictionMap||{};
    const challengerCtx=cfm.overview?`
CHALLENGER SELLING FRICTION MAP:
Natural stages (strength): ${(cfm.naturalStages||[]).join(', ')}
Friction stages:
${(cfm.frictionStages||[]).map(f=>`  ${f.stage}: ${f.whyItsFriction||''}
    What they'll do: ${f.whatTheyWillDo||''}
    Coaching cue: ${f.coachingCue||''}
    What it feels like: ${f.whatItFeelsLike||''}`).join('\n')}
D implication: ${cfm.dominanceImplication||''}
E implication: ${cfm.extroversionImplication||''}
P implication: ${cfm.paceImplication||''}
C implication: ${cfm.conformityImplication||''}
Most important advice: ${cfm.repSpecificAdvice||''}`:''

    // Customer reading guide
    const crg=proscan.customerReadingGuide||{};
    const customerCtx=crg.overview?`
CUSTOMER READING GUIDE (most people want to be communicated with the way they are wired):
Natural fit physician: ${crg.naturalFitPhysician||''}
Friction physician: ${crg.frictionPhysician||''}
Adaptation strategy: ${crg.adaptationStrategy||''}
High-D physician: ${crg.highDPhysicianTip||''}
High-E physician: ${crg.highEPhysicianTip||''}
High-C physician: ${crg.highCPhysicianTip||''}
Steady-Pace physician: ${crg.steadyPacePhysicianTip||''}`:''
...
${traitRangeCtx}${traitPairCtx}${challengerCtx}${customerCtx}${overrideCtx}${calibCtx}`;
  }
...
const server=app.listen(PORT,()=>console.log(`MedDeviceSalesPro API v4.0 running on port ${PORT}`));
