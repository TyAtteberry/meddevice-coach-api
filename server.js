...
// AUTH
// ══════════════════════════════════════════════════════════════
// ── IN-MEMORY SESSION STORE (survives requests, resets on redeploy) ──
...
app.post('/auth/login', (req, res) => {
  ...
  // Generate a token and store in memory
...
app.post('/auth/verify', (req, res) => {
  ...
  // Valid if in memory OR if it matches a HMAC of the password (survives redeploy)
  const valid = SESSION_STORE.has(token) || isStatelessToken(token);
  res.json({ valid });
});

// Stateless token: HMAC of password — always valid regardless of redeploy
...
function isStatelessToken(token) {
  try {
    const expected = makeStatelessToken();
...
function authMiddleware(req, res, next) {
  ...
  // Accept in-memory tokens OR stateless HMAC token
...
// Legacy ElevenLabs proxy (keep for backwards compat)
