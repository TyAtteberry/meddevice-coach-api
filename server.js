// ══════════════════════════════════════════════════════════════
// Conceromed — Backend Server v3.1
...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'3.0'}));
...
app.post('/rep/daily-brief',authMiddleware,async(req,res)=>{
  const{repName,repEmail,products,specialties,territory,proscan,accounts,goals,quarter}=req.body;
  ...
  if(quarter?.goals?.length){
    goalsCtx=`CURRENT QUARTER GOALS:\n${quarter.goals.map(g=>{
      ...
      return `- ${g.text} (${g.progressPct||0}% complete)${overdueTactics.length?' — '+overdueTactics.length+' overdue tactic(s)':''}${todayTactics.length?' — '+todayTactics.length+' due today':''}`;
    }).join('\n')}`;
  }
...
🌟 [One powerful motivational quote relevant to sales or performance — attributed to real person]

✅ WHAT'S LOOKING GOOD
[2-3 sentences about what's going well — reference specific accounts or goal progress if available. Be specific and genuine.]

📋 TODAY'S FOCUS
[2-4 specific things to get done today — reference actual accounts and next steps if available. Numbered list, plain text.]

🔮 ON THE HORIZON
[1-2 sentences about upcoming priorities or deadlines worth noting]

🧠 TODAY'S COACHING
[3-5 sentences of coaching personalized to their ProScan profile. Reference their specific traits. Some days challenge them to get outside their comfort zone. Other days warn them to recover. Make it feel like it knows them personally. Never generic.]

Keep the entire brief under 300 words. Warm, human, direct. No corporate speak.`;
...
app.listen(PORT,()=>console.log(`Conceromed API v3.0 running on port ${PORT}`));
