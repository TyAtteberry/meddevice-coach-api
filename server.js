...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'3.7'}));
...
// Bulk mark/revoke founding members — requires admin password in body
app.post('/rep/founding-member/bulk',async(req,res)=>{
  const{adminPassword,emails,revoke=false}=req.body;
  ...
  for(const raw of emails){
    ...
    if(!email||!email.includes('@')){results.skipped.push({email:raw,reason:'invalid format'});continue;}
    try{
      await sbUpsert('rep_profiles',{email,is_founding_member:!revoke,updated_at:ts});
