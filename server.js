// ══════════════════════════════════════════════════════════════
// Conceromed — Backend Server v4.13
...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'4.16'}));
...
app.post('/admin/proscan-invite', adminAuth, async (req, res) => {
  ...
  if (!email) return res.status(400).json({ error: 'email required' });
  try {
    // Check if a rep_profiles row exists — upsert behaves differently for insert vs update
    const existing = await sbGet('rep_profiles', `?email=eq.${encodeURIComponent(email)}&limit=1`);
    if (existing && existing.length > 0) {
      // Row exists — update only the invite field
      await sbUpsert('rep_profiles', {
        email,
      ...
      // No row yet — create a minimal profile row so the invite field has a home
      await sbUpsert('rep_profiles', {
        email,
        name: '',
        proscan_invited_at: sent ? new Date().toISOString() : null,
