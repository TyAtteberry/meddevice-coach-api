...
const GMAIL_USER           = process.env.GMAIL_USER           || 'ty@conceromed.com';
...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'4.7'}));
...
// ── EMAIL SENDING ────────────────────────────────────────────
// Sends via Gmail SMTP using nodemailer + App Password
// GMAIL_USER and GMAIL_APP_PASSWORD env vars required
...
async function sendEmail({ to, subject, html, text }) {
  ...
  try {
    await transporter.sendMail({
      from: `"Ty Atteberry | Conceromed" <${GMAIL_USER}>`,
      to,
      subject,
      text: text || subject,
      html,
...
// GET /admin/members — list all Pro members
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
  ...
  <tr><td style="background:#ffffff;padding:40px;">
    ...
    <p style="font-size:15px;color:#333;line-height:1.7;margin:0 0 16px;">I spent 20+ years watching great reps fail not because they lacked talent, but because no one coached them the way they were actually wired. I built MedDeviceSalesPro to be the tool I wish I'd had — one that learns your behavioral profile, your products, and your accounts, and coaches you accordingly.</p>
    <p style="font-size:15px;color:#333;line-height:1.7;margin:0 0 24px;">To get the most out of it, here's the order I'd recommend:</p>
    ${productLine}
    ...
    <p style="font-size:14px;color:#555;line-height:1.7;margin:0 0 24px;padding-top:20px;border-top:1px solid #eee;">Reply to this email anytime — you'll reach me directly. I want to know how it's working for you and what would make it better.</p>
    <p style="font-size:15px;color:#333;margin:0;">— Ty Atteberry<br><span style="color:#888;font-size:13px;">Founder, Conceromed</span></p>
  </td></tr>
...
app.post('/rep/welcome-email', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const result = await sendEmail({
      to: email,
      subject: `Welcome to MedDeviceSalesPro, ${name.split(' ')[0]} — here's where to start`,
      html,
