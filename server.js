// ══════════════════════════════════════════════════════════════
// Conceromed — Backend Server v4.18
...
app.get('/',(req,res)=>res.json({status:'Conceromed API',version:'4.21'}));
...
// ══════════════════════════════════════════════════════════════
// ADMIN TOOLS — Profile management, diagnostics, access control
// ══════════════════════════════════════════════════════════════

// GET /admin/rep-detail — full profile + billing + stats for one rep
app.get('/admin/rep-detail', adminAuth, async (req, res) => {
  const { email } = req.query;
  if (!email) return res.status(400).json({ error: 'email required' });
  try {
    const [profileRows, billingRows, accountRows, goalRows] = await Promise.all([
      sbGet('rep_profiles', `?email=eq.${encodeURIComponent(email)}&limit=1`),
      sbGet('rep_billing', `?email=eq.${encodeURIComponent(email)}&limit=1`),
      sbGet('accounts', `?email=eq.${encodeURIComponent(email)}&order=updated_at.desc`),
      sbGet('goals', `?email=eq.${encodeURIComponent(email)}&order=updated_at.desc`),
    ]);
    ...
    res.json({
      ...
      accounts: accountRows.map(a => ({ id: a.id, name: a.name, type: a.type, updatedAt: a.updated_at })),
      goalCount: goalRows.length,
      goals: goalRows.map(g => ({ id: g.id, updatedAt: g.updated_at })),
    });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
// POST /admin/wipe-profile — selectively wipe rep data
// body: { email, wipeProfile, wipeProscan, wipeAccounts, wipeGoals }
app.post('/admin/wipe-profile', adminAuth, async (req, res) => {
  const { email, wipeProfile, wipeProscan, wipeAccounts, wipeGoals } = req.body;
  if (!email) return res.status(400).json({ error: 'email required' });
  const results = [];
  try {
    if (wipeProfile) {
      ...
      results.push('profile fields cleared');
    }
    if (wipeProscan) {
      // Use PATCH for explicit null clears — sbUpsert strips nulls
      await sbPatch('rep_profiles', `?email=eq.${encodeURIComponent(email)}`, {
        proscan: null,
        proscan_uploaded_at: null,
        proscan_invited_at: null,
        proscan_pending: false,
        updated_at: new Date().toISOString(),
      });
      results.push('ProScan data cleared');
    }
    if (wipeAccounts) {
      const accts = await sbGet('accounts', `?email=eq.${encodeURIComponent(email)}`);
      for (const a of accts) {
        await sbDelete('accounts', `?id=eq.${encodeURIComponent(a.id)}`);
      }
      results.push(`${accts.length} accounts deleted`);
    }
    if (wipeGoals) {
      const gs = await sbGet('goals', `?email=eq.${encodeURIComponent(email)}`);
      for (const g of gs) {
        await sbDelete('goals', `?id=eq.${encodeURIComponent(g.id)}`);
      }
      results.push(`${gs.length} goal quarters deleted`);
    }
    console.log(`[ADMIN WIPE] ${email}: ${results.join(', ')}`);
    res.json({ success: true, email, wiped: results });
  } catch(e) {
    console.error('[ADMIN WIPE] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});

// POST /admin/resend-magic-link — send a fresh magic link to any email
app.post('/admin/resend-magic-link', adminAuth, async (req, res) => {
  const { email } = req.body;
  if (!email) return res.status(400).json({ error: 'email required' });
  if (!RESEND_API_KEY) return res.status(500).json({ error: 'Email not configured' });
  try {
    // Generate a magic link via Supabase
    const key = SUPABASE_SERVICE_KEY || SUPABASE_KEY;
    const r = await fetch(`${SUPABASE_URL}/auth/v1/admin/users`, {
      method: 'GET',
      headers: { 'apikey': key, 'Authorization': `Bearer ${key}` },
    });
    // Use Supabase magic link generation
    const mlResp = await fetch(`${SUPABASE_URL}/auth/v1/magiclink`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'apikey': key,
    ...
    if (!mlResp.ok) {
      ...
      throw new Error(`Magic link error: ${err}`);
    }
    console.log(`[ADMIN MAGIC LINK] Sent to ${email}`);
    res.json({ success: true, email });
  } catch(e) {
    console.error('[ADMIN MAGIC LINK] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});

// POST /admin/edit-profile — update rep profile fields directly
app.post('/admin/edit-profile', adminAuth, async (req, res) => {
  const { email, name, company, products, specialties, territory } = req.body;
  if (!email) return res.status(400).json({ error: 'email required' });
  try {
    const updates = { email, updated_at: new Date().toISOString() };
    ...
    await sbUpsert('rep_profiles', updates);
    console.log(`[ADMIN EDIT PROFILE] ${email}`);
    res.json({ success: true, email });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
// POST /admin/extend-expiry — push expiry date for a member
app.post('/admin/extend-expiry', adminAuth, async (req, res) => {
  const { email, expiry } = req.body; // expiry = ISO date string or null for forever
  if (!email) return res.status(400).json({ error: 'email required' });
  try {
    await sbUpsert('rep_billing', {
      email,
      subscription_end: expiry || null,
      updated_at: new Date().toISOString(),
    });
    console.log(`[ADMIN EXTEND] ${email} → ${expiry || 'forever'}`);
    res.json({ success: true, email, expiry });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});

// POST /admin/change-tier — change a member's tier
app.post('/admin/change-tier', adminAuth, async (req, res) => {
  const { email, tier } = req.body;
  if (!email || !tier) return res.status(400).json({ error: 'email and tier required' });
  const validTiers = ['founding_10', 'founding_40', 'comp_30', 'comp_90', 'comp_custom', 'paid', 'none'];
  ...
  try {
    ...
    console.log(`[ADMIN CHANGE TIER] ${email} → ${tier}`);
...
// ══════════════════════════════════════════════════════════════
// JOURNEY MAP TEMPLATES — Workspace-level pipeline configuration
// ══════════════════════════════════════════════════════════════

// GET /workspaces/:id/journey-templates — get all templates for workspace
app.get('/workspaces/:id/journey-templates', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const ws = rows[0];
    res.json({ templates: ws.journey_templates || [] });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});

// POST /workspaces/:id/journey-templates — save a template
app.post('/workspaces/:id/journey-templates', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) return res.status(404).json({ error: 'Workspace not found' });
    const ws = rows[0];
    const templates = ws.journey_templates || [];
    const templateId = template.id || `jt_${Date.now()}`;
    const idx = templates.findIndex(t => t.id === templateId);
    const saved = { ...template, id: templateId, updatedAt: Date.now() };
    if (idx >= 0) templates[idx] = saved;
    else templates.push(saved);
    await sbPatch('workspaces', `?id=eq.${id}`, {
      journey_templates: templates,
      updated_at: new Date().toISOString()
    });
    console.log(`[JOURNEY TEMPLATE] saved "${template.productName}" for workspace ${id}`);
    res.json({ success: true, template: saved });
  } catch(e) {
    console.error('[JOURNEY TEMPLATE] error:', e.message);
    res.status(500).json({ error: e.message });
  }
});

// DELETE /workspaces/:id/journey-templates/:templateId
app.delete('/workspaces/:id/journey-templates/:templateId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) return res.status(404).json({ error: 'Workspace not found' });
    const ws = rows[0];
    const templates = (ws.journey_templates || []).filter(t => t.id !== templateId);
    await sbPatch('workspaces', `?id=eq.${id}`, {
      journey_templates: templates,
      updated_at: new Date().toISOString()
    });
...
// POST /rep/feedback — send feedback email to Ty
app.post('/rep/feedback', authMiddleware, async (req, res) => {
  const { type, typeLabel, text, star, context, starStr, repName, repEmail, version } = req.body;
  if (!text) return res.status(400).json({ error: 'text required' });

  try {
    if (RESEND_API_KEY) {
      const subject = `${typeLabel || '[FEEDBACK]'} from ${repName || 'a rep'} — MedDeviceSalesPro ${version || ''}`;
      const html = `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;max-width:600px;margin:40px auto;color:#333;">
        <div style="background:#0a0c0f;padding:20px 24px;border-radius:8px 8px 0 0;">
          <h2 style="color:#00c9a7;margin:0;font-size:18px;">${typeLabel || 'Feedback'}</h2>
          <p style="color:#9199aa;margin:4px 0 0;font-size:13px;">MedDeviceSalesPro ${version || ''}</p>
        </div>
        <div style="background:#f9f9f9;border:1px solid #eee;border-top:none;padding:24px;border-radius:0 0 8px 8px;">
          ${starStr ? `<p style="font-size:18px;margin:0 0 16px;">${starStr}</p>` : ''}
          <div style="background:#fff;border:1px solid #e0e0e0;border-radius:6px;padding:16px;margin-bottom:16px;white-space:pre-wrap;font-size:14px;line-height:1.7;">${text.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
          ${context ? `<div style="background:#f0f0f0;border-radius:6px;padding:12px;font-size:12px;color:#666;white-space:pre-wrap;">${context.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>` : ''}
          ${repEmail ? `<p style="margin:16px 0 0;font-size:12px;color:#999;">Reply-to: <a href="mailto:${repEmail}">${repEmail}</a></p>` : ''}
        </div>
      </body></html>`;

      await sendEmail({
        to: 'ty@conceromed.com',
        subject,
        html,
        text: `${typeLabel}\n\n${text}${starStr ? '\n'+starStr : ''}${context || ''}`,
    ...
    // Also save to Supabase for searchable log
    try {
      await sbUpsert('feedback', {
        id: `fb_${Date.now()}`,
        type: type || 'general',
        text,
        star: star || null,
    ...
    } catch(e) { /* feedback table may not exist yet — non-fatal */ }

    console.log(`[FEEDBACK] ${typeLabel} from ${repName} (${repEmail})`);
    res.json({ success: true });
  } catch(e) {
    console.error('[FEEDBACK] error:', e.message);
    // Return success anyway — don't block the thank you screen
    res.json({ success: true, note: 'email may not have sent' });
  }
});

// DELETE /accounts/:accountId — permanently remove account and all its activity
app.delete('/accounts/:accountId', authMiddleware, async (req, res) => {
  const { accountId } = req.params;
  const email = req.headers['x-rep-email'] || '';
  if (!accountId) return res.status(400).json({ error: 'accountId required' });
  try {
    // Delete all activity for this account first
    await sbDelete('activity', `?account_id=eq.${encodeURIComponent(accountId)}`);
    // Delete the account itself
    await sbDelete('accounts', `?id=eq.${encodeURIComponent(accountId)}${email ? '&email=eq.'+encodeURIComponent(email) : ''}`);
    console.log(`[DELETE ACCOUNT] ${accountId} by ${email}`);
